...
public class RRAutoCore extends LinearOpMode {
    ...
    /**safe range for IntakeServoAxon = 0.60 */
    ...
    public class AutoIntakeBothWheelsAction implements Action {
        private boolean initialized = false;
        String OnOff;
        double desiredPower;
        ElapsedTime timer;

        public   AutoIntakeBothWheelsAction(String OnOff, double power) {
            this.OnOff = OnOff;
        ...
        public boolean run(@NonNull TelemetryPacket packet) {
            if (!initialized) {
                ...
                if(OnOff == "ON") {
                    ...
                    autoRobot.Intake.intakeLeftWheel.setPower(desiredPower);
                    autoRobot.Intake.intakeRightWheel.setPower(-desiredPower);                    initialized = true;
                } else if (OnOff == "OFF") {
                    autoRobot.Intake.intakeLeftWheel.setPower(0);
                    autoRobot.Intake.intakeRightWheel.setPower(0);
                }
            }
                 return false;
